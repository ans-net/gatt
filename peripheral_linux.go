...
func finish(op byte, h uint16, b []byte) bool {
	...
	if e != attEcodeAttrNotFound {
		// logInfo("unexpected protocol error: %s", e)
...
func (p *peripheral) DiscoverCharacteristics(cs []UUID, s *Service) ([]*Characteristic, error) {
	...
	for !done {
		...
		for len(b) != 0 {
			...
			if s == nil {
				logInfo("Can't find service range that contains 0x%04X - 0x%04X", h, vh)
	...
	go func() {
		for {
			...
			case req := <-p.reqc:
				...
				default:
					logInfo("Request 0x%02x got a mismatched response: 0x%02x", reqOp, rspOp)
					// FIXME: terminate the connection?
				}
				req.rspc <- r
			case <-p.quitc:
